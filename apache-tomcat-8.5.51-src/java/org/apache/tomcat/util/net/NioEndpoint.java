...
public class NioEndpoint extends AbstractJsseEndpoint<NioChannel> {
     ...
     * 主要是监听连接事件,绑定默认端口为8080
     * 使用的是阻模式
     */
    private volatile ServerSocketChannel serverSock = null;

    /**
     *  服务关闭时调用,等待所有的Poller线程关闭
     */
    ...
    public void startInternal() throws Exception {
        ...
        if (!running) {
            ...
            /**
             * 创建Poller线程,最大是2个
             * 不断从队列中(主要是新的加入连接)或者是监听socket中(已经注册过的连接)的事件进行处理
             */
    ...
    protected boolean setSocketOptions(SocketChannel socket) {
        ...
        try {
            ...
            // 从缓存队列弹出一个NioChannel实例,null则创建/否则重置属性
        ...
        public void run() {
            ...
            while (running) {
                ...
                try {
                    ...
                    //如果当前连接数超出 限定的数量 , 则阻塞 , 未超出则数量+1;
                    countUpOrAwaitConnection();
                    //把SocketChannel当成是Socket就行了
                    ...
                    log.info("来了新连接" + "hostName=" + hostName + " port=" + port);
    ...
    public static class PollerEvent implements Runnable {
        ...
        public void run() {
            ...
            if (interestOps == OP_REGISTER) {
                try {
                    log.info("将此channel注册到Poller线程[" + Thread.currentThread().getName()+"]中,后续该channel事件由该线程负责!");
                    // 将此连接的SocketChannel注册到Poller线程上,注册读事件
    ...
    public class Poller implements Runnable {
        ...
        public void run() {
            ...
            while (true) {
                ...
                try {
                    if (!close) {
                        // 执行PollerEvent事件(主要是处理连接注册到当前线程的Selector事件,感兴趣事件为读事件)
                        hasEvents = events();
                ...
                while (iterator != null && iterator.hasNext()) {
                    ...
                    String remoteHost = attachment.getRemoteHost();
                    int remotePort = attachment.getRemotePort();
                    log.info("线程[" + Thread.currentThread().getName() + "]监听到[" + remoteHost + remotePort + "]事件");
                ...
                if ( close ) {
                    ...
                    if (sk.isReadable() || sk.isWritable() ) {
                        if ( attachment.getSendfileData() != null ) {
                            ...
                            // 设置当前channel的感兴趣事件,一般情况下为 0 , 也就是不对任何事件监听
